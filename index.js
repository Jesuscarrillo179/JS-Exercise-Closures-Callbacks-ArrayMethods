...
function processSum(numberList, callback) {
  return callback(numberList.reduce( function(sum, item){
    return sum + item;
  }, 0));
}
...
function firstNamesAllCaps(runners) {
  let capitalRunners = [];
  runners.map((name) => {
    return capitalRunners.push(name.first_name.toUpperCase());
  })
  return capitalRunners;
}
...
function tallyUpDonations(runners) {
  return runners.reduce((donationsAdded, donations) => {
    return donationsAdded + donations.donation;
  }, 0);
}
